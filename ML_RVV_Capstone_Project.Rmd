...
Genre column includes more than one available genre for each movie, so when calculating bias generated by genres it will be necessary to aggregate several biases if a movie belongs to more than one genre. With such purpose I created a couple of specific functions that will be disscussed later and added a new column sum_b_g, initialized to 0 that will contain, $Σb_{g}$, the sum of all genre bias that each movie can be included into and will be calculated when dealing with biases
...
As expected results are still far from being acceptable. It seems clear that some movies tend to be rated better than other so to capture this effect realted to movies, next step will be including bias introduced by movies as follows:
...
Following recommendations made by The BellKor Solution pdf, averages are shrunk towards zero by using regularization. Regularization constrains the total variability of the effect sizes by penalizing large estimates that come from small sample sizes and in order to do so, The BellKor Solution uses the regularization parameters, $λ_1,λ_2$, which are determined by validation on the test set. For this reason I created the functions lambda1 and lambda2 which will allow me to identify the best values of $λ_1,λ_2$. Note that to prevent any unexpected problem I decided to initialize the value of those parameters 
...
The paper made by the “BellKor’s Pragmatic Chaos” final solution states that the users tend to change their baseline ratings over time including a natural drift in a user’s rating scale, and besides, item’s popularity may also change over time. I agree on these assumptions as some of these time effects can be observed on the next plots
...
n <- seq(380, 830, 50)
n1<-sapply(n,bins)
qplot(n,n1) + labs(caption = "Figure 8")
n1<-n[which.min(n1)]
```
...
## Save results to table
Results<-rbind(Results,tibble(method = "Movie Bias reg + User bias reg + time effect bi + dev_u(t)", RMSE = usertimebias_rmse,MAE = usertimebias_mae))
Results
...
## Save results to table
Results<-rbind(Results,tibble(method = "Movie Bias reg + User bias reg + time effect bi + dev_u(t) + Genre bias", RMSE = movusergenrebias_rmse,MAE = movusergenrebias_mae))
Results


```

Comparing Figure 4, Figure 5 and Figure 8 and also RMSE and MAE calculated for each of those process it is possible to have a lazy idea of the influence of each of those bias on final predictions. 
