...
where $|t −t_{u}|$ is the absolute difference between the day of the rating and the average rating date, id est, how far we are from average rating activity done by user 'u',sign is the sign of the subtraction positive or negative, and  α and β parameters to be determined by validation on the test set. In this case I took the same β referred in the paper and then tried to optimize α and β via functions specifically created that I named dev1 and dev2.
...
which sligthly affect the model in a positive way:  
...
I calculate the results of my final model on the validation set 
...
obtaining a final  __`r format(final_rmse , big.mark=",",scientific=F)`__ RMSE 
