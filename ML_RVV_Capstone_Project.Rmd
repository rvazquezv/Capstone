...
where R(i) is the set of ratings done on movie 'i',  |R(i)| the length or cardinality of this subset and $\mu$ the average of all ratings. So for each user 'u' rating movie 'i' I will aggregate the difference between the rating $r_{u,i}$ and the average $\mu$ and instead of dividing by $|R(i)|$ as a normal average would consist of, I add a penalization term on the denominator $λ_{1}$ to be determined

and  the proposed formula for regularizing  $b_{u}$ is:
$$ b_{u}=\frac{Σ_{i∈R(u)}·(r_{u,i}-\mu-b_{i})}{λ_{2} + |R(u)|} $$

where R(u) is the set of ratings done by user 'u',  |R(u)| the  cardinality of this subset, $\mu$ the average of all ratings,$b_{i}$ the effect on movies previouly calculated and  $λ_{2}$ the regularization parameter to calculate  
...
where $t −t_{u}|$ is the absolute difference between the day of the rating and the average rating date ie how far we are from average rating activity done ny user 'u', α and β parameters to be determined by validation on the test set. In this case I took the same β referred in the paper and then tried to optimize α and β via dev1 and dev2 functions
