...
Genre column includes more than one available genre for each movie, so when calculating bias generated by genres it will be necessary to aggregate several biases if a movie belongs to more than one genre. With such purpose I added a new column sum_b_g, initialized to 0 that will contain, $Σb_{g}$, the sum of all genre bias that each movie can be included into and will be calculated when dealing with biases
