...
Once dataset was created, I have based my work on Bell and Koren's papers describing their final solution to the Netflix Prize: http://www.netflixprize.com/assets/GrandPrize2009_BPC_BellKor.pdf.
...
```{r loading-Harvardx-code, message=FALSE}
...
```{r Partitioning 2 edx dataset, message=FALSE}
summary(test_set)
...
where $Y_{u,i}$ are the ratings done by user 'u' on  movie 'and'i', $\mu$ the global average and $\epsilon_{u,i}$ the error committed on prediction done for each couple (user,movie)
...
The distribution of this $b_{i}$ effect can be observed in the following plot:
...
```{r Baseline 2 movies bias results, message=FALSE}
Results
```
...
where R(i) is the set of ratings done on movie 'i',  |R(i)| the length or cardinality of this subset and $\mu$ the average of all ratings. So for each user 'u' rating a movie 'i', I will aggregate the difference between its rating $r_{u,i}$ and the average $\mu$ and instead of dividing by $|R(i)|$ as a normal average would consist of, I add a penalization term on the denominator $λ_{1}$ to be determined later.
...
```{r Baseline 2.1 movies bias + regularization results, message=FALSE}
Results
```


Regularization does not seem to improve too much, but at least improves so I decided to keep it as part of my model.The same as with movie effect (movies bias), there seems to be clear that some users tend to be more critic than others so this effect on users will be the user bias $b_{u}$ and next step will be to calculate it:

$$Y_{u,i}=\mu+b_{i}+b_{u}+\epsilon_{u,i}$$  


Once obtained, the distribution on $b_{u}$ is shown in Figure 5.  
...
The plot in Figure 5 illustrate the distribution of the correction needed from previous model. 
This new model improves like this:

```{r Baseline 3 movies bias reg + user bias results, message=FALSE}
Results
```
...
Similarly,  the proposed formula for regularizing  $b_{u}$ is:
$$ b_{u}=\frac{Σ_{i∈R(u)}·(r_{u,i}-\mu-b_{i})}{λ_{2} + |R(u)|} $$

where R(u) is the set of ratings done by user 'u',  |R(u)| the  cardinality of this subset, $\mu$ the average of all ratings,$b_{i}$ the effect on movies previously calculated and  $λ_{2}$ the regularization parameter to calculate.  
...
```{r Baseline 3.1 movies bias reg + user bias reg results, message=FALSE}
Results
```
...
Figure 6 and Figure 7 are the same plot but at a different scale (from weeks to moths) showing the evolution of average rating across time.
...
Thus following Bellkor's path, I have built first of all time bias $b_{i}$ and secondly user bias $b_{u}$ as functions of time.
...
This formula means that instead of calculating the average bias for each movie and keep it constant all through the time, it is preferable to split it discretely on tranches or bins, and calculating the degradation or improvement of each movie bias during each of those periods.
...
rmse<-m1
n_days<-m
qplot(n_days,rmse) + labs(caption = "Figure 10")
n1<-m[which.min(m1)]
```

so based on this approach I split bins in around `r format(n1,big.mark=",",scientific=F)` days each. To be completely right this  arbitrary number of bins should also be optimized, but due to the computational time taken and small profit obtained I decided to keep these arbitrary ones.
...
where $|t −t_{u}|$ is the absolute difference between the day of the rating and the average rating date, id est, how far the rating done by user 'u' on that day is from his average period of activity, sign is the sign of the subtraction positive or negative, and  α and β parameters to be determined by validation on the test set. In this case I took the same β referred in the paper and then tried to optimize α and β via functions specifically created that I named dev1 and dev2.

The effect of this time dependent inclusion on bias $b_{i}$  can be observed in Figure 11.
...
while results of binning $b_{i}$ are:
```{r Baseline  4.1  movie rating date bias results, message=FALSE}
Results
```
...
```{r Baseline  4.2  α and β, message=FALSE}
...
which sligthly(almost residual) affect the model in a positive way.  It is important to note that at each step improving became more difficult, and the quantity gained in terms of RMSE or MAE is not always worhty in terms of computer resources needed and calculus time.
...
with  $x_{u,i}^{k}$ a binary variable being 1 if the genre for user u's rating of movie i is genre k, 0 otherwise, and  $\beta_{k}$ the effect associated with genre k. 
Thus if the genre match the effect, then it will be applied, otherwise will not. Notice that one movie can belong to several genres at the same time and for this reason the aggregation is needed.
...
Thus, considering 50 eigenvalues we are able to explain `r format(100 * sum(s$d[1:50]^2) / sum(s$d^2), big.mark=",",scientific=F)` % of the variability. So I will re-build my prediction on residuals based on those 50 eigenvalues. 
...
After comparing results obtained by the two approaches explored, it seems obvious that matrix factorization seems promising. Nevertheless the gain obtained in terms of RMSE is only `r format(small_Results$RMSE[2]-small_Results$RMSE[3] , big.mark=",",scientific=F)` which does not seem enough to compensate computational resources needed.

For this reason I decided to choose the linear model "`r Results$method[8]`" as my final model.
...
```{r validation 1, message=FALSE,echo=FALSE}
## Release Year is included in the title, adding a column with the year
...
```
I calculate the results of my final model on the validation set, obtaining a final  __`r format(final_rmse , big.mark=",",scientific=F)`__ RMSE 
...
```{r conclussion , message=FALSE}
Final_Results <- tibble( Final_model=Results$method[8], RMSE = final_rmse, MAE = final_mae)
Final_Results
```
...
but not too much on matrix factorization while Bell and Koren write
    ...
    It seems that models based on matrix-factorization were found to be most accurate (and thus popular), as evident by recent
    publications and discussions on the Netflix Prize forum. We definitely agree to that, and would like to add that those 
    matrix-factorization models also offer the important flexibility needed for modeling temporal effects and the binary view. 
    Nonetheless, neighborhood models, which have been dominating most of the collaborative filtering literature, are still   expected to be popular due to their practical characteristics - being able to handle new users/ratings without re-training and offering  direct explanations to the recommendations.
    
Thus, next steps could be progressing on singular value decomposition to be able to apply SVD++ as Bellkor`s team proposed and exploring neighborhood models and gradient boosting decission trees to finally ensemble the best of them in order to improve my algoritihm
