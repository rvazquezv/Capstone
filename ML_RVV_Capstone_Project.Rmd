...
It is curious to see that despite the size volume, the distributions of the users, movies and ratings on both subset seem quite alike. 
...
where $Y_{u,i}$ are the ratings done by user 'u' on  movie 'and'i' $\mu$ the global average and $\epsilon_{u,i}$ the error commited on prediction done for each couple (user,movie)
...
where R(i) is the set of ratings done on movie 'i',  |R(i)| the length or cardinality of this subset and $\mu$ the average of all ratings. So for each user 'u' rating a movie 'i', I will aggregate the difference between its rating $r_{u,i}$ and the average $\mu$ and instead of dividing by $|R(i)|$ as a normal average would consist of, I add a penalization term on the denominator $λ_{1}$ to be determined
...
Figure 6 and Figure 7 are the same plot but a diffent scale (from weeks to moths) showing the evolution of average rating across time.
...
And finally Figure 9 shows us the number of ratings done each year
...
In their paper they state that to choose the bins they have considered periods of time of around thirty weeks, but these considerations do not work well enough in my case so I created bins function assuming an arbitrary number of 10 bins that optimize the time split between each of them as follows
...
where $|t −t_{u}|$ is the absolute difference between the day of the rating and the average rating date, id est, how far we are from average rating activity done by user 'u', and  α and β parameters to be determined by validation on the test set. In this case I took the same β referred in the paper and then tried to optimize α and β via dev1 and dev2 functions

The effect of this time dependent inclusion on bias $b_{i}$ the optimal values for α and β and the effect of time on bias $b_{u}$ are shown below:
