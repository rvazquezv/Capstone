...
The Netflix Prize was an open competition for the best algorithm to predict user ratings for films, based on previous ratings without any other information about the users or films. BellKor's Pragmatic Chaos team won the contest on 2009 earning US$1,000,000. Through all this process, R. Bell, Y. Koren and C. Volinsky members of the aforementioned team, wrote several papers on their work and  researchs.    
...
Taking a closer look to these formulas, they are almost alike to normal averages but increasing the denominator with a parameter that shrunk the averages toward zero as previously explained.
...
Figure 6 and Figure 7 are the same plot but at a diffent scale (from weeks to moths) showing the evolution of average rating across time.

Figure 8 plots average rating against the number ot times ratings on a film were produced, proving that the more a film is rated the better its rating becomes.
...
rmse<-m1
number_of_bins<-m
qplot(number_of_bins,rmse) + labs(caption = "Figure 10")
n1<-m[which.min(m1)]
```
...
which sligthly(almost residual) affect the model in a positive way.  It is important to note that at each step improving became more difficult, and the quantity gained in terms of RMSE or MAE is not always worhty in terms of computer resources needed and calculus time needed also.
